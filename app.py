...
class Kartya_skinnel:
    def __init__(self, nev, sebzes, eletero, elem, skin):
        self.nev = nev        # Kártya neve
        self.sebzes = sebzes  # Sebzés
        self.eletero = eletero  # Életerő
        self.elem = elem  # Az elem, amihez a kártya tartozik (pl. tűz, levegő stb.)
...
def adatok_kikuldese():
    ...
    window.evaluate_js(f"setPythonData({jatekos_gyujtemeny_json}, {kazamatak_json}, {letezo_kartyak_json}, {vezer_kartyak_json}, {win}, {loss}, '{mod}');")
    ...
    for move in harc_menet:
        ...
        if action == "kijatszik":
            if player_or_enemy == "kazamata":
                ...
                print("KAZAMATA KÁRTYA SKIN:", nagy_kazamatapakli[0].skin)
                window.evaluate_js(f"kozepre_mozgatas('kazamata', {json.dumps(nagy_kazamatapakli[0].skin)});")
                time.sleep(2)
                ...
                print("JÁTÉKOS KÁRTYA SKIN:", nagy_jatekospakli[0].skin)
                window.evaluate_js(f"kozepre_mozgatas('jatekos', {json.dumps(nagy_jatekospakli[0].skin)});")
                time.sleep(2)
            ...
            if player_or_enemy == "jatekos":
                ...
                window.evaluate_js(f"tamadas('jatekos', '{gif_dmg}', '{szin}', '{gif_elem}', '{card_name}', '{step_data[6]}', {json.dumps(nagy_jatekospakli[0].skin)} );")
                ...
                #     table_canvas.delete(dmg_kep)
                # table_canvas.update()
                window.evaluate_js(f"tamadas('kazamata', '{gif_dmg}', '{szin}', '{gif_elem}', '{card_name}', '{step_data[6]}', {json.dumps(nagy_kazamatapakli[0].skin)});")
                while not check_js_status():  # Addig várunk, amíg a művelet be nem fejeződik
                    time.sleep(0.1)  # Várakozunk, majd újra próbálkozunk
                window.evaluate_js("reset_operation_complete();")
        ...
        if len(nagy_jatekospakli) == 0:
            ...
            if ("Új kártya" in jutalom_szoveg[1] or "Fejlesztés" in jutalom_szoveg[1]):
                ...
                window.evaluate_js(f"vege_a_harcnak('{jutalom_szoveg[0]}</br>{jutalom_szoveg[1]}', '{'|'.join(jut)}', {jutalom_szoveg[3]});")
            else:
                print("Jutalom szöveg:", jutalom_szoveg)
                window.evaluate_js(f"vege_a_harcnak('{'|'.join(jutalom_szoveg)}','','alma');")
...
class API:
    ...
    def vilag_kimentese(self, nev, normal_kartyak, vezer_kartyak, tombocske, currentgyujtemeny):
        ...
        try:
            f = open(f"{nev}_world.txt", "w", encoding="utf-8")
            for kartya in normal_kartyak:
                f.write(f"uj kartya;{kartya[0]};{kartya[1]};{kartya[2]};{kartya[3]};{kartya[4:]}\n")
            for vezer in vezer_kartyak:
                f.write(f"uj vezer;{vezer[0]};{vezer[1]};{vezer[2]};{vezer[3]};{vezer[4:]}\n")
            for kazamata in tombocske:
                f.write(f"uj kazamata;{kazamata}\n")
            for sajat in currentgyujtemeny:
                f.write(f"felvetel gyujtemenybe;{sajat}\n")
            f.close()
        except Exception as e:
            print(e)
    ...
    def jatekvilagok_neveinek_betoltese(self):
        try:
            ...
            print(e)
    def karakter_torlese(self, nev):
        print("Karakter törlése:", nev)
        try:
            f = open("karakterek.txt", "r", encoding="utf-8")
            sorok = f.readlines()
            f.close()
            f = open("karakterek.txt", "w", encoding="utf-8")
            for adat in sorok:
                if (adat.strip() == ""):
                    continue
    ...
    def harc_meghivo(self,sajat_pakli, kazamata_ellen):
        ...
        vanEBoss = False
        #print(kapottak)
        print(len(vezer_kartyak),"len(vezer_kartyak)")
        for kartyacska in vezer_kartyak:
            print("KÁRTYACSka NÉV A VEZÉRBŐL:", kartyacska.nev)
            print("KÁRTYACSka ELLENFÉL NÉV A KAZAMATÁBÓL:", kazamata_ellen["ellenfelek"][-1])
            ...
            try:
                ...
                for adat in f.readlines():
                    ...
                    if milyenAdat == "vilag":
                        ...
                        ideiglenes_gyujt.append(adat[0])
                        #jatekos_gyujtemeny.append(Kartya_skinnel(adat[0], int(adat[1]), int(adat[2]), adat[3], adat[4]))
                f.close()
            except Exception as e:
                print(e)
        ...
        try:
            ...
            for i in f.readlines():
                ...
                if adat[0] == "uj kartya":
                    ...
                    elem = adat[4]
                    skin = adat[5]
                    letezo_kartyak.append(Kartya_skinnel(nev, sebzes, eletero, elem, skin))
                    ...
                    elem = adat[4]
                    skin = adat[5]
                    vezer_kartyak.append(Kartya_skinnel(nev, sebzes, eletero, elem, skin))
                elif adat[0] == "uj kazamata":
                    kazamatak.append(Kazamata(adat[1:]))
                elif adat[0] == "felvetel gyujtemenybe" and full_uj_jatek:
                    kartya = None
            ...
            f.close()
            if not full_uj_jatek:
                for kartyaa_adat in letezo_kartyak:
                    if kartyaa_adat.nev in ideiglenes_gyujt:
                        if kartyaa_adat not in jatekos_gyujtemeny:
                            jatekos_gyujtemeny.append(kartyaa_adat)
    ...
    def animacio_betoltese(self):
        ...
        for i in range (2):
            ...
            try:
                ...
                for i in range(1, hanyEmber+1):
                    folder_path = os.path.join(current_directoryy, f"{folder}{i}")
                    animation_folder_path = os.path.join(folder_path, "animation")
                    if os.path.exists(animation_folder_path):
                        for animation_file in os.listdir(animation_folder_path):
                            # Ha nem az 'animalo_seged_vonal.png' fájlt találjuk
                            if  animation_file.endswith(".png"):
                                # Animáció fájl neveinek hozzáadása
                                atvaltott_animation_nev = animation_file
                                if atvaltott_animation_nev:
                                    last_part = folder_path.split("\\")[-1]
                                    kepnevek.append([f"{animation_folder_path.split(os.sep)[-1]}/{animation_file}", last_part])
                                else:
                                    print(f"Nincs megfeleltetés az animációs fájlnévhez: {animation_file}")
        
            except Exception as e:
                print(f"Hiba történt a képek betöltésekor: {e}")
            print("Képek betöltve:", kepnevek)
            nagyanimacionevek.append(kepnevek)
        return nagyanimacionevek
...
def harc(jatekos_pakli_nevek, kazamata):
    ...
    try:
        ...
        def clone_kartya(k):
            if ("skin" in k.__dict__):
                return Kartya_skinnel(k.nev, k.sebzes, k.eletero, k.elem, k.skin)
            else:
                    ...
                    if not kazamata_pakli:
                        ...
                        if kazamata.jutalom != "kartya":
                            if jatekos_orig_refs:
                                ...
                                new_kartya = clone_kartya(orig_kartya)
                            ...
                            if mod == "alap":
                                ...
                                for i,kartya_teszt in enumerate(jatekos_gyujtemeny):
                                    if kartya_teszt.nev == reward.nev:
                                        ...
                                        print(jatekos_gyujtemeny[i].nev, jatekos_gyujtemeny[i].sebzes, jatekos_gyujtemeny[i].eletero, jatekos_gyujtemeny[i].elem)
                                        jutalom_ki = ["Győzelem!","Fejlesztés!",f"{reward.nev};{reward.sebzes};{reward.eletero};{reward.elem}",reward.skin]
                                ...
                                if not marmegvan:
                                    ...
                                    jutalom_ki = ["Győzelem!","Új kártya!",f"{reward.nev};{reward.sebzes};{reward.eletero};{reward.elem}",reward.skin]
